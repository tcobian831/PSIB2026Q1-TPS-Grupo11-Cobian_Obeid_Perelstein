...
# Rutas robustas: funcionan corriendo desde la raíz del repo o desde scripts/
PROJECT_DIR = Path(__file__).resolve().parents[1]
OUTPUT_DIR = PROJECT_DIR / "outputs"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

ENTRADAS_POSIBLES = [
    OUTPUT_DIR / "eeg_data_cargado.parquet",  # recomendado: outputs/
    PROJECT_DIR / "eeg_data_cargado.parquet", # compatibilidad con Script 01 viejo
]

def encontrar_entrada() -> Path:
    """Busca el parquet generado por Script 01 en las ubicaciones esperadas."""
    for ruta in ENTRADAS_POSIBLES:
        if ruta.exists():
            return ruta

    rutas_txt = "\n".join(f"  - {r}" for r in ENTRADAS_POSIBLES)
    raise FileNotFoundError(
        "No se encontró 'eeg_data_cargado.parquet'.\n"
        "Corré primero el Script 01 y verificá que el archivo quede en alguna de estas rutas:\n"
        f"{rutas_txt}"
    )
...
def graficar_señales_tiempo(df: pd.DataFrame):
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "figura_exploracion_tiempo.png", dpi=150, bbox_inches="tight")
    plt.show()
...
def graficar_señales_multicanal(df: pd.DataFrame):
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "figura_exploracion_multicanal.png", dpi=150, bbox_inches="tight")
    plt.show()
...
def graficar_psd(df: pd.DataFrame):
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "figura_exploracion_psd.png", dpi=150, bbox_inches="tight")
    plt.show()
...
def graficar_distribucion_amplitudes(df: pd.DataFrame):
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "figura_exploracion_artefactos.png", dpi=150, bbox_inches="tight")
    plt.show()
